      ...
      FixedDiv(lo.opentop - state.z, dist) >= params.aimslope)
   {
      if(li->special)
      {
         if(params.damage != D_MININT)
            ...
            P_GazeSpecialLine(params.thing, li, lineside);
      }
      return true;
   }
   return false;
   ...
   const divline_t &trace)
{
   ...
   if(in->isaline)
   {
      ...
      {
         if(context.params.damage != D_MININT)
            P_ShootSpecialLine(context.params.thing, li, lineside);
         else
            P_GazeSpecialLine(context.params.thing, li, lineside);
      }
      ...
      if(context.params.damage != D_MININT)
      {
         P_SpawnPuff(x, y, z, P_PointToAngle(0, 0, li->dx, li->dy) - ANG90, updown, true,
   ...
   params(params)
{
   ...
   else
   {
      if(params.damage != D_MININT)
      {
         ...
         state.z = params.thing->z - params.thing->floorclip + (params.thing->height >> 1) +
            8 * FRACUNIT;
      }
      else
      {
